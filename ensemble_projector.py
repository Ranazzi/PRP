...
class EnsembleProjector(object):
    def __init__(self, net, img_shape, latent_dim, size=5000, upscaling_rate=4,
        ...
        if loss_net_folder is not None:
    ...
    def project(self, sample, steps, m_error):
        ...
        z = np.random.normal(0, 1.5, (self.N, self.latent_dim)).T  # generate initial latent ensemble
        ...
        d_real = sample.T.reshape(-1, sample.T.shape[-1])

        if isinstance(m_error, str) or isinstance(m_error, float):
            c_array = np.ones(d_real.shape[0]) * m_error
        else:
            c_array = m_error
        ...
        for i in range(steps):
            ...
            d_pred = d_pred.T.reshape(-1, d_pred.T.shape[-1])

            # d_pred = d_pred.reshape((self.N, 46*69)).T

            ofunc = tf.reduce_sum(tf.square(d_pred - d_real), axis=0).numpy()
